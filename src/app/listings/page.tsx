...
import { Map, Globe, MapPin, Filter, X, Loader2, Wallet, Zap, Droplets, Shield, FileCheck, Landmark } from 'lucide-react';
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
function ListingsContent() {
  ...
  useEffect(() => {
    // Merge static listings with database listings
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="sticky top-16 z-30 border-b bg-background/95 py-4 backdrop-blur shadow-sm">
        <div className="container mx-auto px-4">
          ...
          {(activeFilters.location !== 'all' || activeFilters.category !== 'all' || activeFilters.amenities.length > 0 || activeFilters.titleStatus !== 'all') && (
            <div className="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-muted/50">
               <span className="text-[9px] font-black uppercase text-muted-foreground mr-2">Filtering by:</span>
               {activeFilters.location !== 'all' && (
                 <Badge variant="secondary" className="bg-primary/5 text-primary gap-1">
                   {activeFilters.location} <X className="h-2.5 w-2.5 cursor-pointer" onClick={() => toggleLocation('all')} />
                 </Badge>
               )}
               {activeFilters.category !== 'all' && (
                 <Badge variant="secondary" className="bg-primary/5 text-primary gap-1">
                   {activeFilters.category} <X className="h-2.5 w-2.5 cursor-pointer" onClick={() => toggleCategory('all')} />
                 </Badge>
               )}
               {activeFilters.amenities.map(a => (
                 <Badge key={a} variant="secondary" className="bg-accent/10 text-accent-foreground gap-1">
                   {a} <X className="h-2.5 w-2.5 cursor-pointer" onClick={() => toggleAmenity(a)} />
                 </Badge>
               ))}
               <Button variant="link" onClick={resetFilters} className="text-[10px] h-auto p-0 text-muted-foreground underline">Clear All</Button>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="grid items-center gap-12 lg:grid-cols-2 relative z-10">
            ...
            <div className="flex flex-col sm:flex-row justify-start lg:justify-end gap-6">
              <Button size="lg" className="h-20 px-12 text-lg font-black uppercase tracking-widest bg-primary hover:bg-primary/90 shadow-2xl transition-transform active:scale-95" asChild>
                <a href="https://wa.me/254790011042?text=Hello%20Sunpak%20Estate%2C%20I%20am%20interested%20in%20your%20land%20projects." target="_blank" rel="noopener noreferrer">Talk to an Agent</a>
              </Button>
