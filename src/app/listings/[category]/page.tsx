...
export default function CategoryPage({ params: paramsProp }: CategoryPageProps) {
  ...
  const [activeFilters, setActiveFilters] = useState({ 
    ...
    category: initialCategory as ListingCategory | 'all',
    maxPrice: 10000000,
    amenities: [] as string[],
  ...
  if (initialCategory !== 'all' && !CATEGORIES.find(c => c.id === initialCategory)) {
    notFound();
  }

  const categoryDetails = CATEGORIES.find(c => c.id === activeFilters.category);
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl">
            <Badge variant="secondary" className="mb-4 bg-accent text-accent-foreground font-black uppercase text-[10px] tracking-widest px-3 py-1">
              Comfort Homes Portfolio
            </Badge>
            <h1 className="font-headline text-4xl font-black lg:text-7xl uppercase tracking-tight">
              {activeFilters.category === 'all' 
                ? 'All Land Projects' 
                : categoryDetails?.name}
              {activeFilters.location !== 'all' && ` in ${locationDetails?.name}`}
            </h1>
            <p className="mt-6 text-xl text-primary-foreground/80 font-medium leading-relaxed max-w-2xl">
              {activeFilters.category === 'all' 
                ? 'Explore our diverse portfolio of verified plots across high-growth regions in Kenya.' 
          ...
          <div className="flex flex-wrap items-center justify-between gap-6">
            <div className="flex flex-wrap items-center gap-3">
              <Sheet>
                ...
                <SheetContent side="left" className="w-[340px] overflow-y-auto px-0">
                  <SheetHeader className="px-6 pb-4">
                    <SheetTitle className="font-headline uppercase font-black text-2xl tracking-tighter">Advanced Filters</SheetTitle>
                  </SheetHeader>
                  <div className="px-6 space-y-8 pb-20">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <Button variant={activeFilters.location === 'all' ? 'secondary' : 'outline'} size="sm" onClick={() => toggleLocation('all')} className="justify-start font-bold h-10">
                          <Globe className="mr-2 h-3.5 w-3.5" /> All
                        </Button>
                        {LOCATIONS.map(loc => (
                          <Button key={loc.id} variant={activeFilters.location === loc.id ? 'secondary' : 'outline'} size="sm" onClick={() => toggleLocation(loc.id)} className="justify-start text-[11px] font-bold h-10">
                            {loc.name}
                          </Button>
                        ))}
                      </div>
                      ...
                      <div className="space-y-3">
                        {AMENITY_OPTIONS.map((amenity) => (
                          <div key={amenity.id} className="flex items-center space-x-3">
                            <Checkbox id={amenity.id} checked={activeFilters.amenities.includes(amenity.id)} onCheckedChange={() => toggleAmenity(amenity.id)} />
                            <label htmlFor={amenity.id} className="text-sm font-bold leading-none cursor-pointer flex items-center gap-2">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex gap-2">
                        {['all', 'Ready', 'Freehold'].map((status) => (
                          <Button key={status} variant={activeFilters.titleStatus === status ? 'secondary' : 'outline'} size="sm" onClick={() => setActiveFilters(prev => ({ ...prev, titleStatus: status }))} className="flex-1 font-bold h-10">
                    ...
                    <Button variant="ghost" className="w-full text-muted-foreground font-black uppercase text-[10px] tracking-widest h-12 hover:bg-destructive/10" onClick={resetFilters}>
              ...
              <div className="hidden items-center gap-2 lg:flex">
                <Badge variant={activeFilters.category === 'all' ? 'default' : 'outline'} className="cursor-pointer px-4 py-2 font-bold uppercase text-[10px] tracking-widest" onClick={() => toggleCategory('all')}>
                  All Types
                </Badge>
                {CATEGORIES.map(cat => (
                  <Badge key={cat.id} variant={activeFilters.category === cat.id ? 'default' : 'outline'} className="cursor-pointer px-4 py-2 font-bold uppercase text-[10px] tracking-widest" onClick={() => toggleCategory(cat.id)}>
                    {cat.name}
                  </Badge>
                ))}
              </div>
            ...
            <p className="text-xs font-black uppercase tracking-widest text-muted-foreground bg-muted/40 px-3 py-2 rounded-lg">
              {dbLoading ? <Loader2 className="h-3 w-3 animate-spin inline mr-2" /> : null}
              Showing <span className="text-primary">{filteredListings.length}</span> verified plots
            </p>
          </div>
        </div>
      </div>
          ...
          {filteredListings.length > 0 ? (
            ...
            <AnimatedSection>
              <div className="flex flex-col items-center justify-center rounded-[3rem] bg-muted/30 py-32 text-center border-2 border-dashed border-muted/50">
                <MapPin className="mb-6 h-16 w-16 text-muted-foreground/20" />
                <h2 className="text-3xl font-black uppercase tracking-tight">No plots found</h2>
                <p className="mt-2 text-muted-foreground font-bold uppercase text-[10px] tracking-widest max-w-xs mx-auto">
                  Try adjusting your location, price, or amenity filters to see more results.
                </p>
                <Button variant="outline" className="mt-10 font-black uppercase text-xs h-14 px-10 border-primary/20" onClick={resetFilters}>
                  Clear All Filters
                </Button>
              </div>
            </AnimatedSection>
      ...
      <section className="container mx-auto px-4 pb-20">
        <div className="overflow-hidden rounded-[3rem] bg-accent p-10 text-accent-foreground lg:p-20 shadow-xl relative">
          <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/2" />
          <div className="grid items-center gap-12 lg:grid-cols-2 relative z-10">
            <div>
              <h2 className="font-headline text-4xl font-bold lg:text-6xl leading-tight">Can't find your <span className="text-primary">dream plot?</span></h2>
              <p className="mt-8 text-xl text-accent-foreground/80 font-medium leading-relaxed">
                Our property experts have access to unlisted gems and upcoming projects. 
                Talk to us today and let's find your dream plot together.
              </p>
            </div>
            <div className="flex justify-start lg:justify-end">
              <Button size="lg" className="h-20 px-12 text-lg font-black uppercase tracking-widest bg-primary text-primary-foreground hover:bg-primary/90 shadow-2xl transition-all active:scale-95" asChild>
                <a href="https://wa.me/254790011042?text=Hello%20Sunpak%20Estate%2C%20I%20am%20interested%20in%20your%20land%20projects." target="_blank" rel="noopener noreferrer">Talk to a Land Expert</a>
              </Button>
            </div>
          </div>
        </div>
